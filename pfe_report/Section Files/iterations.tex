...
Two safeguards protect each bank's data: the database itself blocks any attempt to read another bank's records, and the privileged administrative actions re-check the user's role on the server on every request. The validation argument of this iteration is a two-layer enforcement argument, walked, as in the previous iterations, by tracing the worst case of each threat through the design before accepting it. The two layers are not two independent checks stacked over the same operation, and so are not defence-in-depth in the strict sense; they divide the work across two classes of operation. Row-level security in the database is the sole authoritative check for the ordinary read and write path. The privileged service-role endpoints bypass row-level security by design, so for those operations the server code is the sole authoritative check rather than a second line behind the database.
