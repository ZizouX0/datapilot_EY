...
\begin{sloppypar}
Above the database sits the serverless layer: five POST endpoints, \texttt{/api/invite}, \texttt{/api/set-role}, \texttt{/api/manage-user}, \texttt{/api/set-department}, and \texttt{/api/update-self}, each a thin handler delegating to a framework-agnostic core module. The same core modules are mounted as middleware on the development server, so development and production share one code path. All five hold the service-role key on the server only. The deployment view of the platform, with the single-page application, the serverless endpoints, and Supabase under row-level security, is shown in Figure~\ref{fig:deployment}.
\end{sloppypar}
