...
Beyond the rule boundaries, a representative complete session of all 47 indicators reproduced the illustrative profile detailed in Chapter 5: a Global Maturity Index of 2.19 out of 5, displayed as 44\% and Level 2, Emerging. Recomputing the weighted sum by hand from the per-dimension scores confirmed the displayed index matches the methodology of Iteration 2 exactly. The captured demonstration session of Figures~\ref{fig:screen-results} through~\ref{fig:screen-compliance} is a second complete session, also within the emerging band, retraced the same way; closing and reopening the browser restored it from local storage, confirming persistence. Because this validation is scenario based rather than automated, it confirms the exercised paths but cannot rule out untested edge cases; the absence of an automated test suite is a limitation taken up in future work. Within that boundary, the evaluation confirmed that the tool reproduces the scoring methodology faithfully and presents the results in an actionable form, addressing research question RQ3, with the consolidated results presented in Chapter 5. The evaluation also delimited the artifact---one assessor, one browser, one bank, the record held only in that browser---and turning this instrument into a platform a whole bank can operate under EY administration is the problem the fourth iteration addresses.
