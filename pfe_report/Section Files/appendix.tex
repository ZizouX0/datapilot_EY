...
The Governance dimension carries the heaviest weight and the largest share of the regulatory indicators.
...
The Skills and Culture dimension is assessed indirectly, through eight proxy indicators across two sub-dimensions that can be verified against objective organizational evidence.

\noindent \textit{All indicators in D5 are proxy indicators. They rely on proxy evidence that is largely objective and auditable (headcount ratios, certification counts, budget lines, and program documentation). Some D5.2 rubrics, however, also involve qualitative judgments of culture and engagement (for example, whether the business drives the data agenda). These judgments are proxies for the culture itself rather than direct measurements of it.}
...
This appendix maps each of the thirteen non-skippable regulatory indicators to the specific obligation of the BCT Circulaire N\textdegree{}2025-08 or the BCBS~239 principles that it operationalizes. The circular's data-governance obligations are set out in its Article~6, which is the anchoring article for the rows attributed to the BCT below; the rows attributed to BCBS~239 derive instead from that standard's principles. The obligations described in the table are the author's own reading of these texts, pending validation by a compliance or legal authority, and are stated as read from the source rather than quoted authoritatively. This is consistent with the limitation noted in the Introduction. These indicators are flagged as \textbf{[BCT]} in Appendix~A and cannot be skipped in any assessment. They are concentrated in Governance (seven), Data Quality (three), Architecture and Access (two), and Analytics and Tools (one); Skills and Culture carries none.
...
\hline
Access rights (RBAC) & D3.2 & BCT: role-based access control with logged and periodically reviewed access grants and complete audit trails. \\
\hline
...
This appendix consolidates, for the delivered multi-tenant platform, which of the four roles may perform each platform capability. The four roles form a strict rank order: EY Admin (owner) above Super Admin above Admin above Analyst. Each role is summarized below in rank order, followed by the detailed capability matrix.

\begin{description}
\item[EY Admin (owner)] The platform owner at EY, with no bank of its own. Day to day, it oversees all banks, maintains the EY master questionnaire template, and reviews submissions across banks. It invites each bank's Super Admin.
\item[Super Admin] Runs a single bank and sees only that bank. Day to day, it manages the bank's departments, user accounts, and roles, coordinates group assessments, and reviews the bank's submissions; the questionnaire content is read-only for this role. It invites Admins.
\item[Admin] Operates the assessments inside its bank. Day to day, it tailors the bank's questionnaire copy, maps dimensions to departments, launches and finalizes group assessments, and reviews the bank's submissions. It invites Analysts.
\item[Analyst] The contributor. Day to day, it runs its own solo assessments, which it can submit for review, and contributes the dimensions assigned to its department in a group assessment. It invites no one.
\end{description}

\noindent Two rules hold uniformly across the four roles:
\begin{itemize}
  \item Every invitation creates an account exactly one rank below the inviter; no invitation can create a peer or a superior.
  \item These permissions are not enforced by the interface alone. Every capability is re-checked on the server, which re-reads the caller's role and bank from the database on each request, and row-level security confines every database row to its own bank. A tampered client therefore cannot exceed the caller's role or reach another bank's data.
\end{itemize}

\noindent Table~\ref{tab:role-matrix} details, capability by capability, which role may perform each action. A check mark ($\checkmark$) means the role may perform the capability; a dash (--) means it may not.
...
\hline
View results, gap analysis, and compliance of a finalized submission & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
\hline
...
\hline
indicators & Per-bank copy of the 47 indicators, with regulatory flags and rubrics & Bank name (empty = master) \\
\hline
