...
This section presents the iterations through which the artifacts of the project were produced. Following the Design Science Research regulative cycle, each iteration is structured into five activities: problem investigation, solution design, design validation, solution implementation, and solution evaluation. Four iterations are presented. The first designs the data maturity assessment framework, the second constructs the scoring methodology, the third develops the DataPilot tool that operationalizes them, and the fourth scales the tool into a multi-tenant platform through which EY administers the instrument across banks.

The four iterations follow a single progression that can be named in four words: Define, Measure, Steer, Scale. Iteration 1 defines what data maturity means for a Tunisian bank by fixing the dimensions, sub-dimensions, and indicators against which every institution is assessed; Iteration 2 defines how that maturity is measured by turning the forty seven indicator scores into a single auditable index; Iteration 3 turns the measurement into a steering instrument by embedding it in an interactive tool; and Iteration 4 scales the instrument from one assessor to the institution and to the sector, turning the single-user tool into a role-based, tenant-isolated platform in which one bank means many contributors and EY oversees many banks. Each iteration consumes the artifact of its predecessor as its input, and the reader may hold this progression as a reference point throughout the narratives that follow. Table~\ref{tab:iterations-overview} summarizes the four iterations, the artifact each delivers, the rules or components that carry it, and the outcome of its evaluation.

\begin{table}[H]
\centering
\caption{Overview of the four DSR iterations}
\label{tab:iterations-overview}
{\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{|p{1.8cm}|p{3.0cm}|p{5.4cm}|p{4.2cm}|}
\hline
\textbf{Iteration} & \textbf{Artifact delivered} & \textbf{Key rules or components} & \textbf{Key outcome} \\
\hline
1 Define & Maturity assessment framework & Five dimensions, twelve sub-dimensions, forty seven indicators with five level rubrics; weights 25/20/20/20/15; thirteen BCT indicators flagged & Coverage and regulatory alignment confirmed; D5 retained as proxy only \\
\hline
2 Measure & Scoring methodology & Weighted renormalized aggregation to the GMI; evidence cap; skip ceilings of two, two, one, one, one; non-skippable regulatory indicators & Worked example of 2.19 out of 5, 44\%, Level 2 Emerging; evidence cap verified \\
\hline
3 Steer & DataPilot tool & Client-side single-page application with six screens and a completion gate & Functional testing on simulated and anonymized data confirmed fidelity to the methodology \\
\hline
4 Scale & Multi-tenant platform & Four roles (EY Admin (owner), Super Admin, Admin, Analyst) with invite-only onboarding; bank-scoped tenant isolation by row-level security; departments and group assessment; per-bank questionnaire copies from the EY master template & Functional scenarios on the seeded demo bank confirmed role gates, tenant isolation, and finalize locking \\
\hline
\end{tabular}
}
\end{table}

\subsection{Overview of DataPilot}

DataPilot is today a multi-tenant platform in which four roles collaborate. The EY Admin (owner) administers the EY master template of the questionnaire and reviews submissions across banks; each bank's Super Admin and Admin manage the bank's users, departments, and group assessments, with the Admin additionally tailoring the bank's copy of the questionnaire; and Analysts run the assessment workflow itself, either individually as a solo assessment or collectively through a group assessment in which each department answers its assigned dimensions. The assessment core remains what Iterations 1 to 3 built: a guided questionnaire that collects the indicator scores with their rubrics and evidence, an automated scoring engine, and Results, Gap Analysis, and Compliance views dedicated to the maturity dashboard, the prioritized gaps, and the regulatory posture. A completion gate guards the Results, Gap Analysis, and Compliance views: any attempt to reach them before every dimension of the questionnaire is complete redirects the user back to the questionnaire. The identity of the respondent now comes from the invited account rather than from a typed profile form. The four iterations below progressively build the conceptual framework, the scoring engine, the single-user tool, and the platform.

A single DataPilot session takes a bank from zero to a complete, BCT-auditable maturity assessment through five sequential steps: (1) Institution Setup, where the bank and the respondent are identified from the analyst's account and the assessment is configured; (2) Indicator Evaluation, where each of the forty seven indicators is scored using the per-level rubric and supported with documentary evidence; (3) Automated Scoring, where the scoring engine applies the weighted aggregation formula and all business rules; (4) Results Dashboard, where the Global Maturity Index (GMI) on a scale of one to five, the corresponding percentage, and the Gartner label are presented with the radar visualization; and (5) Gap Analysis and Compliance, where priority gaps, the improvement roadmap, and the BCT compliance view are displayed. The final output of a session consists of the Global Maturity Index score, the Gartner executive label, a BCT compliance score, a prioritized action roadmap, and an exportable evidence package. This journey describes the solo assessment path and is traced step by step in Figure~\ref{fig:user-journey}.
...
The full set of interactions supported by the platform, across the four roles EY Admin (owner), Super Admin, Admin, and Analyst, is captured in the use case diagram of Figure~\ref{fig:usecase}.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{images/usecase.pdf}
...
The path that a raw indicator response follows through the scoring engine can be read as a data flow with four transformations. A raw response, consisting of a score from one to five together with an optional evidence text and a skipped flag, first passes through the effective score computation, which applies the evidence cap and excludes skipped or unanswered indicators. The surviving effective scores are then averaged within each sub-dimension, the sub-dimension scores are averaged within each dimension, and the dimension scores are combined by the renormalized weighted sum that yields the Global Maturity Index, from which the percentage and the maturity level are finally derived. This data flow is shown in Figure~\ref{fig:data-flow}, which makes explicit where each business rule intervenes: the evidence cap and the skip and non-skippable rules act at the level of the individual response, while renormalization acts at the final aggregation step.
...
The maturity levels are reported on the five level scale derived from CMMI, complemented by the Gartner labels, as summarized in Table~\ref{tab:maturity-levels}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{images/activity.pdf}
\caption{User flow and completion gate of DataPilot}
\label{fig:activity}
\end{figure}

An assessment session can also be described as a state machine, which makes the completion gate explicit as a condition on a transition rather than a screen. A session begins in an unconfigured state and moves to a configured state once the profile has been saved with the required bank name and respondent name. From the configured state it enters an in-progress evaluation state, in which indicators are scored, evidenced, or skipped; the session remains in this state until every dimension is complete, at which point the completion gate opens and the session reaches a complete state from which the results, gap analysis, and compliance views become reachable. Because the assessment is persisted to local storage, the session can be suspended and resumed at any point without losing its state, and a reset returns it to the unconfigured state. This lifecycle is shown in Figure~\ref{fig:assessment-lifecycle}.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/assessment-lifecycle.pdf}
\caption{State machine of a DataPilot assessment session}
\label{fig:assessment-lifecycle}
\end{figure}

\subsubsection{Solution Implementation}

The tool was implemented with the technologies described in Section 3. Its six screens realize the assessment workflow end to end, and each is shown as a live screenshot of the Iteration 3 build of the application in Figures~\ref{fig:screen-welcome} through~\ref{fig:screen-compliance} at the end of this subsection. In the Iteration 3 build, the Welcome screen introduces DataPilot as the data maturity steering tool and collects the session profile through an inline form, the bank name, the assessment date, the respondent name, and the role, whose start action stays disabled until all four fields are filled and then opens the Questionnaire directly. The Profile screen allows the same details to be reviewed or edited at any point during the session, and it also exposes a guarded reset that clears the stored assessment. The Questionnaire screen is the core data entry surface: it presents the forty seven indicators grouped by dimension and sub-dimension, and for each indicator it displays the question, the guidance hint, the five level rubric as selectable options, an evidence field, and a skip control; it also shows the scoring rules and an overall progress indicator so the respondent always knows how much of the assessment remains. The skip control is disabled for the thirteen regulatory indicators and becomes unavailable once a dimension has reached its skip limit, and the evidence field is what determines whether a high score survives the evidence cap; a skipped indicator displays an undo control that restores it to the answerable state and frees the corresponding skip within the dimension ceiling, and a refused skip is explained by an explicit message naming the dimension and its ceiling. The Results dashboard presents the maturity profile through a Recharts radar chart that overlays the current score of each dimension on the selected target level, together with the Global Maturity Index (GMI) expressed both on the five point scale and as a percentage, the maturity level with its CMMI name and Gartner label, the computation formula displayed for auditability, and an interpretation grid stating the recommended action for each maturity level. The Gap Analysis screen ranks the twelve sub-dimensions by ascending score, assigns each a priority label from critical to low, and positions the five dimensions on an effort and impact matrix whose quadrants, quick wins, major projects, fill ins, and low priority, are derived from each dimension score and its weight; from the same scores it derives a three phase improvement roadmap, with a first phase of zero to three months dedicated to critical gaps and BCT compliance, a second phase of three to six months dedicated to formalization and documentation, and a third phase of six to twelve months dedicated to optimization, each populated with the recommended actions of the dimensions it contains. The Compliance screen reports a status per regulatory indicator, compliant when the effective score reaches three out of five, non compliant below that threshold, and pending when unanswered, together with the supporting evidence, the overall compliance rate computed as the share of the thirteen indicators that are compliant, and the resulting regulatory exposure level, which is rated low when the rate reaches eighty percent, medium when it reaches fifty percent, and high otherwise.
...
\caption{Sequence of the BCT regulatory compliance computation}
\label{fig:sequence-compliance}
\end{figure}

The six screens of the Iteration 3 build are shown below. Figure~\ref{fig:screen-welcome} shows the Welcome screen and Figure~\ref{fig:screen-profile} the Profile form. Figure~\ref{fig:screen-questionnaire} shows the Questionnaire, the core data entry surface. Figures~\ref{fig:screen-results}, \ref{fig:screen-gap}, and \ref{fig:screen-compliance} show the Results dashboard, the Gap Analysis screen, and the BCT Compliance screen for a representative assessment. The Results, Gap Analysis, and Compliance captures were produced from a complete demonstration session of the forty seven indicators, which is why the completion gate allows them to render.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{images/screen-welcome.png}
\caption{DataPilot Welcome screen (single-user build of Iteration 3)}
...
\caption{DataPilot Profile screen for recording the institution and respondent (single-user build of Iteration 3)}
\label{fig:screen-profile}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{images/screen-questionnaire.png}
\caption{DataPilot Questionnaire screen, showing an indicator with its rubric, guidance hint, evidence field, and skip control}
\label{fig:screen-questionnaire}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{images/screen-results.png}
\caption{DataPilot Results dashboard for a representative complete assessment, showing the Global Maturity Index, the dimension radar, and the score interpretation grid}
...
The fourth decision turns the framework content into governed data without making the method negotiable. The forty seven indicator framework of Iteration 1 is loaded as the EY master template, the reference instrument of the platform: the EY Admin maintains the master, each bank receives a private bank copy cloned from it, and the bank's Admin may tailor that copy, adjusting dimension names, weights, descriptions, indicator questions, hints, five level rubrics, regulatory flags, and structure. Super Admins are deliberately read-only on content. Master edits seed future bank copies only; existing copies are frozen clones. Crucially, the scoring rules stay hard-coded and non-negotiable in every tenant: the evidence cap, the skip ceiling of twenty percent per dimension rounded down, the non-skippable status of regulatory-flagged indicators, the compliance thresholds, namely an effective score of at least three with exposure rated low from eighty percent and medium from fifty, the five maturity bands, and the five level scale itself. In one sentence: the content became governed data; the method did not.

The fifth decision retains the solo assessment. Analysts keep the individual assessment path of Iteration 3, with in-progress answers still held locally in the browser, now bound to the signed-in account so that a different user on the same machine cannot read or continue them. A completed solo assessment can additionally be submitted for review, which records an immutable submission row visible to the bank's administrators.

\subsubsection{Design Validation}

The validation argument of this iteration is a two-layer enforcement argument, walked, as in the previous iterations, by tracing the worst case of each threat through the design before accepting it.

\begin{sloppypar}
The first and authoritative layer is row-level security in the Postgres database itself. Every table policy funnels through a small set of security-definer helper functions, among them \texttt{is\_owner()}, \texttt{is\_admin()}, \texttt{is\_superadmin()}, \texttt{is\_bank\_admin()}, \texttt{is\_analyst()}, \texttt{my\_bank()}, and \texttt{my\_department()}, and the tenant isolation pattern is uniform across tables: a row is accessible when the caller is the EY Admin or when the row's bank name equals the caller's own bank. The \texttt{profiles} table carries no client write policy at all, so the browser cannot write a role even while holding the public key.
\end{sloppypar}

The second layer is the small set of privileged serverless endpoints holding the service-role key. Each role-gated endpoint, covering invitations, role changes, account management, and department assignment, re-reads the caller's profile from the database on every request and never trusts a client-sent role; the self-service and roadmap endpoints verify the caller's session token but grant no role-dependent capability. Route guards exist in the client as well, but they are user experience only and carry no security weight; the design records this honestly rather than counting them as a defense.
...
\begin{sloppypar}
Submissions were validated as immutable. The \texttt{submissions} table has no update policy, so neither an analyst nor an administrator can alter a result after submission; deletion is limited to the owner of the row and the bank's administrators; and the insert policy compares the submission's bank name to the caller's own bank, so a submission cannot be forged into another tenant.
\end{sloppypar}
...
\begin{sloppypar}
Above the database sits the serverless layer: six POST endpoints, \texttt{/api/invite}, \texttt{/api/set-role}, \texttt{/api/manage-user}, \texttt{/api/set-department}, \texttt{/api/update-self}, and \texttt{/api/roadmap}, each a thin handler delegating to a framework-agnostic core module, and the same core modules are mounted as middleware on the development server, so development and production share one code path. All six hold the service-role key on the server only, and the roadmap endpoint additionally guards the optional AI text generation behind authentication so that it cannot be consumed anonymously. The deployment view of the platform, with the single-page application, the serverless endpoints, and Supabase under row-level security, is shown in Figure~\ref{fig:deployment}.
\end{sloppypar}
...
\begin{sloppypar}
On the client, the state layer grew from the single store of Iteration 3 to eight Zustand stores: \texttt{useAppStore} (the solo assessment, with the engine unchanged), \texttt{useAuthStore} (session, role, and identity hygiene on account switch), \texttt{useContentStore} (the questionnaire content, loaded through a fallback chain of the bank copy, then the EY master template, then the bundled defaults), \texttt{useAssessmentStore} (the group assessment), \texttt{useUsersStore}, \texttt{useDepartmentsStore}, \texttt{useSubmissionsStore}, and \texttt{useSettingsStore} (the language preference). The scoring engine was extracted into a pure-function module, \texttt{src/lib/scoring.js}, so that the group finalization computes scores identically to the solo path, and the shared \texttt{AssessmentRunner} component renders both the solo questionnaire and the group contributor view with the same rubric cards, evidence field, cap warning, and skip ceiling. The layered architecture of the platform, with the four front-end layers of Iteration 3 now resting on a backend layer, is shown in Figure~\ref{fig:architecture-layers}: solo in-progress answers persist in the browser storage, while submissions, group assessments, profiles, and questionnaire copies persist in Postgres under row-level security.
\end{sloppypar}
...
The stack of Iteration 3 is unchanged, React 19, Vite 8, React Router 7, Zustand 5, Tailwind CSS 4, Recharts 3, and react-to-print 3, extended by the Supabase JavaScript client 2 and the serverless functions; the platform deploys as a static single-page application plus serverless functions on a Vercel-style host, with the environment split between public build-time keys and server-only secrets. Finally, the deterministic roadmap builder of Iteration 3 is unchanged; an optional serverless endpoint can rewrite only the text of the recommended actions through an OpenAI-compatible model endpoint, and the client falls back to the built-in static recommendations on any failure.
...
The interactions summarized in the use case diagram of Figure~\ref{fig:usecase} are specified in detail below. Each use case is grounded in the actual behavior of the tool, including the completion gate, the evidence cap, the skip limit, and the non-skippable status of the regulatory indicators. Use cases UC-01 to UC-11 (Tables~\ref{tab:uc01} to~\ref{tab:uc11}) specify the assessment workflow as it was built in Iteration 3, and they remain valid for the solo assessment path of the platform, with the difference that the profile fields of UC-01 are now supplied by the analyst's invited account and shown read-only on the Welcome screen. Two actors participate in them: the Assessor, typically a data or compliance officer who enters the responses, and Management, who consults the resulting dashboards. The Assessor performs the data entry use cases, while both actors consult the results, the roadmap, and the compliance view. The questionnaire itself is not a forced linear wizard: it can be traversed freely through the dimension tabs and the sub-dimension pills, each showing its completion status, and the completion gate is the only ordering constraint in the tool. Three further use cases cover the lifecycle of the session itself: the guarded reset, the undoing of a skip, and the resumption of a persisted session (UC-09 to UC-11). Use cases UC-12 to UC-16 (Tables~\ref{tab:uc12} to~\ref{tab:uc16}) then cover the platform interactions introduced by Iteration 4, where the actor set grows from the Assessor and Management to the four platform roles: EY Admin (owner), Super Admin, Admin, and Analyst.

\begin{table}[H]
\centering
\caption{UC-01 Configure assessment}
\label{tab:uc01}
...
\begin{table}[H]
\centering
\caption{UC-02 Score an indicator with evidence}
\label{tab:uc02}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
\textbf{Actor} & Assessor \\
...
\begin{table}[H]
\centering
\caption{UC-03 Skip a non-regulatory indicator}
\label{tab:uc03}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
\textbf{Actor} & Assessor \\
...
\hline
\textbf{Alternate flows} & 1a. If the indicator is one of the thirteen regulatory indicators, the skip control is unavailable and the request is refused. 1b. If the dimension has already reached its skip limit, computed as its indicator count multiplied by twenty percent and rounded down, the request is refused. \\
\hline
...
\begin{table}[H]
\centering
\caption{UC-04 View maturity results}
\label{tab:uc04}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
...
\begin{table}[H]
\centering
\caption{UC-05 Review improvement roadmap}
\label{tab:uc05}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
\textbf{Actor} & Assessor, Management \\
...
\hline
\textbf{Main flow} & 1. The actor opens the Gap Analysis screen. 2. The tool ranks the sub-dimensions by ascending score and assigns each a priority label. 3. The tool derives the three phase roadmap, selecting the recommended actions of each dimension from the band that matches its score. \\
\hline
...
\begin{table}[H]
\centering
\caption{UC-06 View BCT compliance}
\label{tab:uc06}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
\textbf{Actor} & Assessor, Management \\
...
\begin{table}[H]
\centering
\caption{UC-07 Export evidence package}
\label{tab:uc07}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
\textbf{Actor} & Assessor \\
...
\begin{table}[H]
\centering
\caption{UC-08 Set target maturity level}
\label{tab:uc08}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
\textbf{Actor} & Assessor, Management \\
...
\begin{table}[H]
\centering
\caption{UC-09 Reset the assessment}
\label{tab:uc09}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
\textbf{Actor} & Assessor \\
...
\begin{table}[H]
\centering
\caption{UC-11 Resume a persisted session}
\label{tab:uc11}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
\textbf{Actor} & Assessor \\
...
\begin{table}[H]
\centering
\caption{UC-12 Invite a user (per-role)}
\label{tab:uc12}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
...
\begin{table}[H]
\centering
\caption{UC-13 Edit the bank questionnaire}
\label{tab:uc13}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
...
\hline
\textbf{Postconditions} & The bank's tailored copy is what its analysts see, and all scores are computed live against it. \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{UC-14 Map dimensions to departments and launch a group assessment}
\label{tab:uc14}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
...
\begin{table}[H]
\centering
\caption{UC-15 Contribute assigned dimensions}
\label{tab:uc15}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
...
\begin{table}[H]
\centering
\caption{UC-16 Finalize and submit a group assessment}
\label{tab:uc16}
\begin{tabular}{|p{3cm}|p{12cm}|}
\hline
\textbf{Actor} & Admin or Super Admin (coordinator) \\
...
This section presented the four iterations through which the framework, the scoring methodology, the DataPilot tool, and the multi-tenant platform were designed, validated, implemented, and evaluated, each following the DSR regulative cycle. The next section presents the consolidated results and findings and answers the research questions.
